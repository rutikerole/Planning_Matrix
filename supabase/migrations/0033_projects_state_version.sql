...
--
-- RECONCILIATION PROVENANCE (v1.0.25, 2026-05-24):
-- The `state_version` column AND its `projects_bump_state_version`
-- trigger are ALREADY LIVE in production as of 2026-05-24 — confirmed
-- empirically in docs/SCHEMA_DRIFT_RECONCILIATION.md (column present
-- via OpenAPI + 200/400 probe; values non-zero/varying 0→10 across
-- prod rows ⇒ trigger firing). They were applied to prod from the
-- parked branch feature/v1-0-6-race-fix (commit 95c8c30) but the
-- migration file was never merged to main, leaving main unable to
-- reproduce the prod schema (FULL_GERMANY_AUDIT.md Bug 39).
-- This commit reconciles main with prod via an IDEMPOTENT re-apply —
-- ADD COLUMN IF NOT EXISTS + CREATE OR REPLACE FUNCTION + DROP/CREATE
-- TRIGGER — so a clean rebuild from main now matches prod, and a
-- re-run in prod is a safe no-op that also GUARANTEES the trigger
-- exists (the one piece PostgREST could not confirm).
--
-- ⚠ APPLY VIA SUPABASE SQL EDITOR ONLY. Do NOT `supabase db push` —
-- the repo has duplicate 0004_*/0005_* migration prefixes
-- (FULL_GERMANY_AUDIT.md Bug 41) that break the CLI runner. Apply
-- this file by pasting into Dashboard → SQL Editor → Run.
-- ───────────────────────────────────────────────────────────────────────
--
