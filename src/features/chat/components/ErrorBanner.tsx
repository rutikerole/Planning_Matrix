// ───────────────────────────────────────────────────────────────────────
// Phase 3 — ErrorBanner
//
// Surfaces above the workspace when the most recent chat-turn failed
// with a non-rate-limit error. Reads from chatStore.lastError, looks
// up `chat.errors.<code>.title` + `chat.errors.<code>.body` from
// i18n. Unknown codes fall through to the `internal` copy. Same
// quiet-bar register as OfflineBanner / RateLimitBanner.
// ───────────────────────────────────────────────────────────────────────

import { useTranslation } from 'react-i18next'
import { AnimatePresence, m, useReducedMotion } from 'framer-motion'
import { useChatStore } from '@/stores/chatStore'

const KNOWN_ERROR_CODES = new Set([
  'streaming_failed',
  'model_response_invalid',
  'upstream_overloaded',
  'upstream_timeout',
  'persistence_failed',
  'idempotency_replay',
  'validation',
  'unauthenticated',
  'not_found',
  'internal',
])

export function ErrorBanner() {
  const { t } = useTranslation()
  const reduced = useReducedMotion()
  const lastError = useChatStore((s) => s.lastError)
  const setLastError = useChatStore((s) => s.setLastError)

  const code = lastError?.code
  const lookupCode = code && KNOWN_ERROR_CODES.has(code) ? code : 'internal'

  return (
    <AnimatePresence>
      {lastError && (
        <m.div
          initial={reduced ? false : { opacity: 0, y: -4 }}
          animate={{ opacity: 1, y: 0 }}
          exit={reduced ? { opacity: 0 } : { opacity: 0, y: -4 }}
          transition={{ duration: reduced ? 0 : 0.2, ease: [0.16, 1, 0.3, 1] }}
          role="alert"
          className="bg-paper border-b border-destructive/30"
        >
          <div className="mx-auto max-w-[1440px] px-6 sm:px-10 lg:px-14 py-2 flex items-center gap-3">
            <p className="flex-1 text-[12px] text-clay/85 leading-snug">
              <span className="font-medium text-destructive/90">
                {t(`chat.errors.${lookupCode}.title`)}
              </span>{' '}
              <span className="italic">
                {t(`chat.errors.${lookupCode}.body`)}
              </span>
            </p>
            <button
              type="button"
              onClick={() => setLastError(null)}
              className="text-[11px] uppercase tracking-[0.16em] text-clay/85 hover:text-ink underline underline-offset-4 decoration-clay/55 transition-colors duration-soft focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ink/35 focus-visible:ring-offset-2 focus-visible:ring-offset-background rounded-sm"
              aria-label={t('chat.banner.errorDismiss', { defaultValue: 'Schließen' })}
            >
              {t('chat.banner.errorDismiss', { defaultValue: 'Schließen' })}
            </button>
          </div>
        </m.div>
      )}
    </AnimatePresence>
  )
}
