...
## D23 · Code hygiene pass — Phase 4.0 audit follow-through (Phase 4.1 · 2026-04-29)

**Question:** Phase 4.0 audited the codebase across the 12 categories in the brief and shipped three high-value must-fix commits (SEO infra + Vercel security headers + tsconfig strict; audit findings doc + audit/ artifacts; mechanical lint cleanup taking ESLint from 27 errors / 26 warnings → 19 errors / 1 warning). The remaining §A.before-beta items split into two buckets — code-able items (C1 inline component definitions, C2 conditional hooks, C3/C4/C5 setState-in-effect / ref-during-render / impure-during-render, C6/C7 mechanical lint cleanup, S3 per-route SEO, SEC2 application-level rate limit, item 10 Playwright smoke suite) and operational/design items (Lighthouse runs, axe-core runs, real-device walk, OG PNG, privacy/ToS, prod Supabase, custom domain, Sentry signup) that genuinely require Rutik's environment. How do we close the code-able half without a second audit cycle and without faking the operational half?

**Answer:** Eight commits (#120–#127) under autonomous-mode authority. ESLint goes from 19 errors / 1 warning → **0 errors / 0 warnings**. Hooks anti-patterns: C1 hoists three inline component definitions (InputBar's Outer wrapper, AttachmentChip's pickIcon-derived `<Icon>`, MessageAttachment's mirror) to module scope, eliminating the focus-loss / re-mount risk on the chat input bar; C2 fixes two rules-of-hooks violations (AttachmentPicker's useEffect-after-early-return, ChatWorkspacePage's late useViewport); C3/C4/C5 converts 3 localStorage hydration sites to `useState(() => readFromStorage())` lazy initial state, refactors EditableCell to derive draft from editing state instead of two-effect prop sync, lifts Thread's read-and-write-ref-during-render to a useState-lazy snapshot, and replaces ChatWorkspacePage's impure `Date.now()` inside useMemo with a useState-lazy mount-time capture. The five remaining "react to external Zustand-owned signal" sites (FactTicker, ThinkingIndicator, MessageContextSheet, Typewriter, the rec-count peek effect in ChatWorkspacePage) get targeted block-form `/* eslint-disable react-hooks/set-state-in-effect */` with multi-line comments explaining why the pattern is correct. C6 mechanical: drops `let outputSize: number | null = null` for definite-assignment-inferred `let outputSize: number` in ExportMenu. C7 extracts non-component constants to sibling `*.helpers` modules (StatusPill.helpers.ts, RoleGlyphs.helpers.ts) so component files only export components (Fast-Refresh friendly). S3 ships per-route document titles + descriptions via React 19's native metadata hoisting (chosen over react-helmet-async to save the dep — React 19 hoists `<title>` / `<meta>` automatically with proper unmount semantics): a single `<SEO />` component reads `seo.title.*` / `seo.description.*` keys from the locale files, the router wraps each Route's element in a fragment with `<SEO />` for static-title routes, and the four project routes wire `<SEO />` from inside the page so the title interpolates the live project name; the SessionGuard also keeps `<html lang>` in sync with i18n.resolvedLanguage. SEC2 ships migration 0008 + an `increment_chat_turn_rate_limit` SECURITY DEFINER RPC + 50/hr per-user cap enforced inside the chat-turn Edge Function with a 429 + structured rateLimit envelope; SPA surfaces this via a new RateLimitBanner that mirrors OfflineBanner's quiet-bar register and shows the reset time in the user's locale. Item 10 ships a focused 4-spec Playwright smoke suite (landing / auth / seo / i18n) plus a GitHub Actions workflow — full coverage matrix (chat round-trip, file upload, share-token anon, axe-core, mobile-layout horizontal-scroll asserts) is sized as Phase 4.2 per `tests/smoke/README.md`.

**Reasoning:** "Honest minimalism" was the discipline carried over from Phase 4.0. The Phase 4.0 sign-off explicitly approved carving deferred items rather than faking them; Phase 4.1 ships exactly the code that can ship in a single autonomous batch — leaves the operational items for Rutik. ESLint at 0/0 is the §A.before-beta gate the audit framed as the v1-readiness signal. Two operational gates land in this batch and require Rutik to apply: migration 0008 (rate-limit table + RPC) needs the Supabase SQL Editor before chat-turn redeploys, and the Playwright suite needs `npx playwright install --with-deps` before its first local run. Both are documented inline in the migration header and `tests/smoke/README.md`. Bundle held green throughout (140.2 KB gzipped, ceiling 250); locale parity grew from 617 → 640 keys (+21 SEO keys + 2 rate-limit banner keys), gate ✓.

---

## D22 · The completion pass (Phase 3.9 · 2026-04-28)

**Question:** Phase 3.8 shipped the mobile foundation (viewport meta fix, three-dimensional token cascade, `useViewport` / `useSafeAreaInsets` / `useKeyboardHeight` / `useNetworkInfo`, `<MobileFrame>`, `<TouchTarget>`, `<MobileChatWorkspace>`, the cockpit-as-card-list, the bundle-size gate). The gesture primitives (`useSwipeGesture`, `useLongPress`) existed but were never wired. The result-page sections used responsive Tailwind that *technically* stacked on mobile but visibly cramped at 320–360 px. Touch targets across the post-3.8 result + chat surfaces were inconsistent — some at 28 px, some at 36, some at 44. Inputs across the working surfaces were 13–15 px which triggers iOS zoom-on-focus. How do we close the gap between "foundation in place" and "actually feels native on mobile" without scope creep?

**Answer:** Twelve commits (#91–#102) under autonomous-mode authority. No migrations, no new edge functions, no new dependencies. Net code: ~1,700 LOC dead-code deletion + ~300 LOC new behaviour (the swipe row + the context sheet + the archive mutation) + many small per-surface tweaks. New: `MobileAuthShell` for the six auth pages so they get a centered single-column register on mobile (the desktop split-screen photo layout would crush at 360 px); wizard polish (pt-safe header, 48 px primary submit on mobile, thumb-stack flex-col-reverse); `SwipeableProjectRow` + `useSetProjectStatus` mutation that wires Phase 3.8's `useSwipeGesture` onto the dashboard so swipe-left reveals Archivieren/Wiederherstellen with the action-button on the right edge; `MessageContextSheet` (vaul drawer) wired via `useLongPress` onto both `MessageUser` + `MessageAssistant` for tap-and-hold copy; LegalLandscape rows stack vertical on mobile (label + status header line + full-width relevance bar below); DocumentChecklist columns become native `<details>`/`<summary>` accordions on mobile so a 12-card "Erforderlich" pile doesn't bury "Liegt vor"; CostTimelinePanel phases stack vertical (label+range above, hatched bar below). Touch-target sweep brought every interactive on the post-3.8 surfaces to ≥44 × 44 px on mobile (RiskFlags, TopThreeHero, SuggestionChips, ScheduleSection, MessageAttachment, AttachmentChip, ConfirmDialog). Zoom-on-focus floor: every text input/textarea bumped to 16 px on mobile (InputBar, InputText, InputAddress, SuggestionChips address input, CockpitTable filter, EditableCell, RiskFlags resolve input, ExportHub share-URL); auth fields already used `text-base`. Dashboard header gains `pt-safe`; ConfirmDialog confirm/cancel bumped h-10 → h-11 on mobile. SwipeableProjectRow action button gained a contextual aria-label. Cleanup: deleted `AdaptiveAnimation`, `useNetworkInfo`, `components/ui/{accordion,button,tabs}` shadcn scaffolds, `GradientMesh`, `PaperCheckbox`, `BinderTabStrip`, `AuditTimeline`, and the five Phase-3.4 structured-input shells (`InputText`, `InputAddress`, `InputMultiSelect`, `InputYesNo`, `SuggestedReplies`) that the Phase 3.6 SuggestionChips pivot retired but never deleted.

**Reasoning:** "The completion pass" framing is the discipline. The Phase 3.8 brief explicitly footnoted that gesture wiring + per-section Mobile* variants were deferred — Phase 3.9's job was to honour those footnotes without re-opening the foundation. The plan-IS-the-sign-off authority granted at the start of the batch trades the mid-batch confirmation gates for the same plan-first discipline that has shipped clean across 3.2–3.8: each commit was scoped against the brief's deliverable list, ships under the locale-parity gate (617 keys, ✓) and the bundle-size gate (still 136 KB gzipped initial — no regression). Where the brief overstated scope, the commits ship the actual delta and the message documents the reduction (e.g. #92's wizard "MobileWizardPage" — the existing `grid-cols-1 sm:grid-cols-2` already stacked the chips, so the commit shipped 11 LOC of targeted polish rather than ceremonial duplication; #95 kept the existing `grid-cols-1 md:grid-cols-3` layout but added native disclosure semantics that the brief's "kanban-to-vertical" framing suggested). The MobileTopBar removal item from the brief was *not* shipped — investigation showed it's still actively used in the desktop branch as a `lg:hidden` tablet-landscape fallback for `ChatWorkspacePage`; deleting it would have broken the 640–1023 px viewport range. The cleanup commit (#101) deleted ~1,700 LOC of orphaned components verified unreferenced via grep across the entire `src/` tree, with each candidate's archaeological provenance written into the commit message so future archaeology has the breadcrumb. The locale parity gate caught two i18n adds in flight (#93 archive/restore, #97 contextSheet); the bundle gate stayed under ceiling. **Phase 3.9 closes the mobile work that 3.8 started; from this commit forward the mobile testing protocol in `docs/mobile-support.md` is the source of truth for what "good" feels like on a real device.**
