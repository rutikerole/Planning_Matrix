...
- **Daily gates:** only `test.yml` runs per-commit (build + Playwright); build transitively runs locales/hardcoded-de/legal-config/bundle. **`verify:bayern-sha` is NOT in CI** despite a P0 runbook classification (Bug 48). The canonical "4 daily gates" (`SESSION_RESUME_STATE.md:246`, `OPS_RUNBOOK.md:34`) are bayern-sha, smoke:citations, tsc, build — **3 of 4 are manual-only**; "all four green" is a human attestation, not CI-enforced.
