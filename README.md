...
- **Phase 3.8** — mobile first, pixel perfect: viewport meta finally carries `viewport-fit=cover` + `interactive-widget=resizes-content` (every prior `env(safe-area-inset-*)` call was a no-op until this); three-dimensional CSS-token cascade (deliverable / operating × desktop / mobile); foundational hooks `useViewport` / `useSafeAreaInsets` / `useKeyboardHeight` / `useNetworkInfo`; `<MobileFrame>` / `<TouchTarget>` / `<AdaptiveAnimation>` / `<SkipLink>` primitives; dedicated `MobileChatWorkspace` orchestrator (collapsing top header, sticky compact progress band, fixed-bottom keyboard-aware input bar); `MobileAttachmentSheet` with camera-capture / gallery / file flow; cockpit table card-list at <640 px via internal viewport branch (one change unlocks all 8 tabs); result-page mobile spacing compression + ConfidenceRadial scale-down; gesture primitives `useSwipeGesture` (iOS edge-back-buffer aware) + `useLongPress`; bundle splitting cuts main entry chunk 232 → 136 KB gzipped (-41 %); build-time bundle-size gate at 250 KB ceiling. Eight commits (`docs/phase3-8-plan.md`). Mobile is the primary surface from this batch forward.
- **Phase 3.9** — the completion pass: closes every Phase 3.8 footnote without re-opening the foundation. `MobileAuthShell` for the six auth pages; wizard polish (pt-safe header, 48 px primary submit, thumb-stack); `SwipeableProjectRow` + `useSetProjectStatus` mutation wiring `useSwipeGesture` onto dashboard rows for swipe-to-archive; `MessageContextSheet` (vaul drawer) wired via `useLongPress` on chat bubbles for tap-and-hold copy; result-page section variants (LegalLandscape vertical-stack rows, DocumentChecklist `<details>`/`<summary>` accordions on mobile, CostTimelinePanel vertical phase rows); touch-target sweep (≥44 px on mobile across post-3.8 surfaces); zoom-on-focus floor (every input ≥16 px on mobile); a11y audit (contextual aria-labels, ConfirmDialog hits, drawer titles); cleanup of ~1,700 LOC of orphaned components (Phase-3.4 input shells, Phase-3.8 AdaptiveAnimation, shadcn scaffolds). No migrations, no new edge functions, no new dependencies. Twelve commits (`docs/phase3-9-plan.md`).
- **Phase 4.0** — production readiness audit: senior-reviewer / first-time-user / adversarial-user lenses across 12 categories. Three must-fix commits shipped (robots.txt + sitemap + Vercel security headers + tsconfig strict; mechanical lint cleanup taking ESLint from 27 → 19 errors; comprehensive findings doc at `docs/phase4-readiness-audit.md`). Verdict: GREEN-LIGHT-WITH-CAVEATS for invited beta. RLS verified across all five tables, share-token validation correct, no committed secrets, no `any`, npm audit clean.
- **Phase 4.1** — code hygiene pass: closes every code-able §A.before-beta item from the Phase 4.0 audit. Eight commits (#120–#127). All hook anti-patterns fixed (C1 inline component definitions, C2 conditional hooks, C3/C4/C5 setState-in-effect / ref-during-render / impure-during-render); per-route SEO via React 19 native metadata (no helmet dep); application-level rate limit on chat-turn (50 turns/hour per user, migration 0008 + RPC + `RateLimitBanner`); first Playwright smoke suite (4 specs across 4 browser/viewport projects) wired into GitHub Actions CI. **ESLint clean: 0 errors / 0 warnings.** Bundle 140.2 KB gzipped (ceiling 250). Locale parity 640 keys. Operational gate: Rutik must apply migration 0008 + redeploy chat-turn (`docs/phase4-1-plan.md`).

## Testing

Smoke suite via Playwright covers landing / auth / per-route SEO / locale persistence on Desktop Chrome + Desktop Safari + Pixel 5 + iPhone 13. CI runs the suite on every push to `main` and every PR (`.github/workflows/test.yml`). Local:

```sh
npx playwright install --with-deps   # ~200 MB browsers, first time only
npm run test:e2e                     # full suite
npm run test:e2e:ui                  # debug UI
```

Coverage matrix + deferred specs in [`tests/smoke/README.md`](tests/smoke/README.md).

## Mobile support

Phase 3.8 makes mobile the primary surface; Phase 3.9 closes the completion pass. See [`docs/mobile-support.md`](docs/mobile-support.md) for supported devices, known limitations, and the testing protocol.
